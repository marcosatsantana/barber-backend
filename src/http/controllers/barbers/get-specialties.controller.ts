...
export async function getBarberSpecialtiesController(req: FastifyRequest, reply: FastifyReply) {
  try {
    await req.jwtVerify()
  } catch (err) {
  ...
  try {
    ...
    const specialtyNames = barber.specialties.map(bs => bs.specialty.name)

    return reply.status(200).send({ 
      specialties: specialtyNames 
    })
  } catch (error) {
    console.error('Error fetching barber specialties:', error)
